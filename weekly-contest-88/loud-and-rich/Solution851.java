...
public class Solution851 {
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(solution851.loudAndRich(new int[][]
    ...
    public int[] loudAndRich(int[][] richer, int[] quiet) {
        ...
        List<List<Integer>> dj = new ArrayList<List<Integer>>(){{
            for (int i = 0; i < len; i++) {
                add(new LinkedList<>());
            }
        }};
        ...
        while (!queue.isEmpty()){
            ...
            for (Integer index : dj.get(cur)) {
                if(quiet[ans[index]]>quiet[ans[cur]])
                    visited[index]=false;
                if(!visited[index]){
                    queue.offer(index);
                    if(quiet[ans[index]]>quiet[ans[cur]])
